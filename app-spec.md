...
```jsonc
{
  ...
  "chat": {
    "mode": "dock"               // "normal"(기존) | "dock"(앱+하단 챗) | "hidden"(앱만 — 상태바 `디버그 채팅` 버튼으로 필요할 때만 꺼냄)
  },
...
처음 만들 때는 `"dock"`을 권한다. 챗이 남아 있어야 사용자가 AI와 대화하며 디버깅할 수 있다. 플레이 입력이 전부 앱에서 이뤄지는 게임형이면 완성 후 `"hidden"`으로 바꿔라 — 채팅은 기본 숨김이 되고, 상태바의 `디버그 채팅` 버튼으로만 열린다(새로고침하면 다시 닫힘).
