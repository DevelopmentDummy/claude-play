...
- **Shadow Root 접근**: `document.currentScript.getRootNode()`을 사용하라. 패널 렌더러가 이 패턴을 Shadow Root 참조(`shadow`)로 자동 치환한다. 옵셔널 체이닝(`?.`) 버전도 지원되지만, 명확성을 위해 일반 호출을 권장한다. IIFE `(function(){ ... })()` 래퍼 안에서도 치환된 `shadow`는 클로저로 접근 가능하다.
