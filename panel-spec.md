...
이 문서는 **글로벌 source of truth**다. 위치: `/panel-spec.md` (저장소 루트).

페르소나·세션 디렉토리에도 같은 이름의 사본이 있다 (`data/personas/<name>/panel-spec.md`, `data/sessions/<name>/panel-spec.md`). 사본은 페르소나/세션 생성 시점의 스냅샷이며, **이후 본 문서가 갱신되어도 자동 동기화되지 않는다.**

### 갱신 시 필수 절차

이 문서를 수정하면 **반드시 글로벌 + 작업 중인 페르소나·세션의 사본까지 함께** 업데이트한다.

1. **글로벌부터 갱신** — `/panel-spec.md` (이 파일)
2. **현재 작업 중인 페르소나** — `data/personas/<name>/panel-spec.md`
3. **현재 작업 중인 세션** — `data/sessions/<sessionId>/panel-spec.md`
4. (선택) 다른 페르소나·세션의 사본도 필요하면 동기화. 다만 페르소나마다 커스텀 섹션이 추가됐을 수 있으므로 *맹목적 덮어쓰기 금지* — 변경 부분만 패치한다.

기존 세션의 사본은 갱신 안 해도 동작에는 영향 없지만, 그 세션 내에서 문서를 참조하는 AI는 옛 정보를 기반으로 판단할 수 있다. 작업 중인 세션은 반드시 동기화해서 일관성을 유지한다.

### 사본을 두는 이유

- 페르소나 단위로 문서를 커스터마이즈할 수 있어야 함 (특정 페르소나의 패널 컨벤션 추가 등)
- 세션이 시작된 시점의 스펙을 보존 — 추후 글로벌이 변해도 세션은 *계약된 시점의 규약*을 따라가야 일관성이 유지됨
- AI가 세션 디렉토리만 보고도 패널 시스템을 이해할 수 있어야 함

### 신규 항목·함정·검증 규칙 추가 시

특히 *함정·실수 방지*에 해당하는 항목(예: "X 방식으로 했다가 안 됐고 Y 방식이 맞다")은 발견 즉시 본 문서에 박아둔다. 같은 실수를 다음 사람·다음 세션·미래의 자기 자신이 반복하지 않도록.
...
AI는 **서사와 감정**을, 엔진은 **상태 판정**을, 패널은 **표시와 입력**을 담당한다. 이 역할 분리는 타협하지 않는다. 자세한 원리는 `panel-design` 스킬의 "아키텍처 원리 — 레이어 책임 경계" 섹션을 참조하라.

### 엔진 중심 아키텍처

**핵심 아이디어:** 분산된 개별 변수 조작 대신, 하나의 **엔진 스크립트**(`tools/engine.js`)가 게임/서사 상태 변이의 허브 역할을 한다. **엔진을 돌리는 주체는 패널이다.** AI는 엔진을 직접 호출하지 않고, 그 결과가 `[STATE]` / `[ACTION_LOG]` / 도메인 헤더로 다음 턴 입력에 주입되면 그걸 읽고 서사를 쓴다.

```
패널 버튼 또는 <choice>.actions → executeAction → engine → 데이터 갱신 → 패널 자동 반영
                                                        ↘ queueEvent → [TIME]/[DOMAIN] 헤더
                                                        ↘ [STATE]/[ACTION_LOG] 자동 주입
                                                                            ↓
                                                          다음 유저 메시지 상단에 첨부 → AI 서사
```

AI는 이 헤더들을 **사실의 원본**으로 받아들이고, 유저 자연어 텍스트를 재실행 명령으로 해석하지 않는다. 상태 변경 호출 경로는 패널 한 곳으로 좁혀 두어야 중복 실행·유령 상태 변경을 구조적으로 막을 수 있다.
...
엔진은 **flat 방식**과 **wrapped 방식** 두 가지 인자 형식을 모두 지원한다:
- **flat** (권장): `{ action: "use_item", item: "이름" }` — `action`과 파라미터를 같은 레벨에 배치
- **wrapped**: `{ action: "use_item", params: { item: "이름" } }` — 파라미터를 `params` 객체로 분리

**패널에서는 flat 방식을 권장한다.** 간결하고 기존 패널 대부분이 이 방식을 사용한다.

**AI (MCP 툴) 에서:**
AI는 **엔진을 직접 호출하지 않는다.** 상태 변경은 유저가 누른 버튼·선택지가 패널을 통해 실행한다. AI는 결과로 주입되는 `[STATE]` / `[ACTION_LOG]` / 도메인 헤더를 읽고 서사만 쓴다. 세션의 `CLAUDE.md`에는 이 원칙을 명시한다:

```markdown
<!-- session-instructions.md 예시 -->
## 상태 관리 규칙
- 엔진 호출은 패널이 수행한다. AI는 `[STATE]` / `[ACTION_LOG]` / `[TIME]` 등 자동 주입 헤더를
  사실로 읽고 서사를 쓴다. 유저 자연어 텍스트는 재실행 명령이 아니라 해설이다.
- 조회성 호출(`query_status` 등)은 꼭 필요한 경우에만 한정하여 사용한다.
```

**패널에서 (권장 패턴 — registerAction + executeAction):**
```javascript
// 1. 핸들러 등록 — 엔진 호출 + AI 브리핑을 한 곳에 모은다
...
// 2. 버튼은 executeAction으로 위임 — 인라인 runTool 금지
shadow.querySelector('.use-potion-btn').addEventListener('click', async () => {
  await __panelBridge.executeAction('use_item', { item: '회복포션' });
});
```

`executeAction` 경로는 `[ACTION_LOG]`에 자동 기록된다. 인라인 `runTool`을 직접 쓰면 AI 시야 밖의 유령 상태 변경이 된다.
...
### 응답 중 패널 잠금 (`layout.panels.lockDuringStreaming`)

기본적으로 AI 스트리밍 중에는 모달/도크 패널 위에 투명한 차단 오버레이가 깔려 클릭이 막힌다(`cursor: not-allowed`). 이는 응답이 끝나지 않은 상태에서 패널 상호작용이 다음 턴과 충돌하는 것을 막기 위한 안전장치다.

페르소나별로 이 동작을 끌 수 있다 — `layout.json`의 `panels.lockDuringStreaming` (boolean, 기본 `true`). `false`로 두면 응답 중에도 패널을 자유롭게 조작할 수 있다.

```json
{
  "panels": {
    "placement": { ... },
    "lockDuringStreaming": false
  }
}
```

| 값 | 외부 오버레이 | `__panelBridge.isStreaming` |
|----|--------------|------------------------------|
| `true` / 미설정 (기본) | streaming 중 클릭 차단 | 정상 노출 |
| `false` | 차단 없음 | 정상 노출 (패널 스크립트가 자체적으로 활용 가능) |

이 옵션은 **외부 오버레이만** 끈다. 패널 스크립트 내부에서 `__panelBridge.isStreaming` 값으로 버튼을 직접 비활성화하는 로직은 그대로 동작하므로, 응답 중에도 일부 액션만 막고 싶다면 패널 측에서 명시적으로 처리한다.
...
배열이나 객체의 항목 수가 늘어나도 deps를 손대지 않으려면 `pluck`을 사용한다. 컬렉션의 각 항목에서 지정한 키만 뽑아 문자열로 합쳐 의존성을 만든다.

```html
<!-- 배열 girls.girls의 모든 항목 title을 의존성으로 -->
<!-- deps: {{pluck girls.girls "title"}} -->

<!-- 여러 키를 동시에 추적 -->
<!-- deps: {{pluck girls.girls "id" "title" "level"}} -->

<!-- 객체에도 사용 가능 (Object.values 사용) -->
<!-- deps: {{pluck rooms "name" "occupied"}} -->
```

새 항목이 추가되거나 기존 항목의 해당 키가 바뀔 때만 트리거되며, 추적하지 않는 다른 속성 변경은 무시된다. 전체 직렬화가 필요하면 `{{json girls.girls}}`를 쓰면 되지만, 사소한 속성 변경도 모두 트리거되므로 가능한 한 `pluck`으로 좁히는 것이 좋다.
...
### A) 선택지 → 패널 액션 + 이벤트 큐 (권장)

**선택이 상태 변경을 동반하는 경우의 권장 패턴.** 핸들러를 `registerAction`으로 등록하고, 버튼은 `executeAction`으로 위임한다. `executeAction`은 `[ACTION_LOG]`에 자동 기록되므로 AI가 무엇이 실행됐는지 안다. 핸들러 내부에서 `queueEvent`로 결과 요약까지 얹으면 다음 유저 메시지에 헤더로 포함된다.
...
<script>
  // 1) 핸들러 등록 — 엔진 호출 + 이벤트 브리핑을 한 곳에
  __panelBridge.registerAction('apply_choice', async (params) => {
    await __panelBridge.runTool('engine', { action: 'apply_choice', choice: params.choice });
    __panelBridge.queueEvent('[선택] ' + params.choice + '을(를) 선택했습니다');
  });

  // 2) 버튼은 executeAction으로만 위임
  shadow.querySelectorAll('[data-choice]').forEach(function(el) {
    el.addEventListener('click', async function() {
      await __panelBridge.executeAction('apply_choice', { choice: el.dataset.choice });
    });
  });
</script>
```

**`sendMessage` vs 패널 액션:**
| | `sendMessage` | `executeAction` (+ queueEvent) |
|---|---|---|
| AI 턴 소모 | ✅ 즉시 AI 응답 트리거 | ❌ 다음 유저 메시지까지 대기 |
| 상태 반영 | AI가 처리 (비결정적) | 엔진이 즉시 처리 (결정적) |
| ACTION_LOG | ❌ 별도 기록 없음 | ✅ `[ACTION_LOG] panel.action` 자동 기록 |
| 사용자 흐름 | 강제 턴 삽입 | 자연스러운 대화 흐름 유지 |
| 적합한 경우 | 단순 대사 전송, 상태 변경 없음 | 점수/인벤토리/스탯 변경 동반 |

**언제 어떤 패턴을 쓸 것인가:**
- 선택이 **상태를 바꾸는** 경우 → `registerAction` + `executeAction` + `queueEvent` (권장)
- 선택이 단순히 **대사를 보내는** 경우 → `sendMessage`
- **인라인 `runTool` 직접 호출은 금지** — `[ACTION_LOG]` 미기록으로 AI 시야 밖 유령 동작이 된다
...
<script>
  // 1) 핸들러 등록 — 엔진 호출 + 실패 처리 + 이벤트 브리핑을 한 곳에
  __panelBridge.registerAction('buy_item', async (params) => {
    ...
    if (!res.result?.success) {
      throw new Error(res.result?.message || '구매 실패');
    }
    await __panelBridge.queueEvent('[상점] ' + params.item + ' 구매 완료');
    return res;
  });
  ...
  shadow.querySelectorAll('.buy-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
      btn.disabled = true;
      try {
        await __panelBridge.executeAction('buy_item', { item: btn.dataset.item });
      } catch (err) {
        btn.disabled = false;
        __panelBridge.showToast(err.message || '구매 실패');
      }
    });
  });
</script>
...
서버에서 직접 결정적 로직을 실행하는 스크립트. 주 호출자는 **패널**(`runTool`)이며, AI의 MCP `run_tool` 경로는 기술적으로 가능하지만 RP 페르소나에서는 상태 변경 용도로 사용하지 않는다(조회성 호출만 예외). 상태 변경은 패널의 `registerAction`/`executeAction` 경로를 거쳐야 `[ACTION_LOG]`에 기록되고 AI가 인지할 수 있다.
...
>
> 체크리스트:
> - 적어도 하나의 버튼이 `sendMessage` / `closeModal` / `__modals` 패치 중 하나로 패널을 닫거나 다음 단계로 넘어가게 한다
...
### `__modals` 패치 시 닫는 규칙 (엔진 액션·도구 결과)

엔진 도구 결과의 `variables.__modals` 패치는 **명시된 키만 처리**된다. 키를 *제거*해서 닫으려는 시도는 무시된다.

```js
// ❌ 잘못된 닫기 — API가 '훈련세부' 키 변경을 인지 못 함, 기존 'dismissible' 유지
const next = { ...(v.__modals || {}) };
delete next['훈련세부'];
return { variables: { __modals: next }, ... };

// ✅ 올바른 닫기 — false로 명시 설정해야 닫힘
return { variables: { __modals: { '훈련세부': false } }, ... };

// ✅ 올바른 열기 — 다른 모달 상태는 건드리지 않고 해당 키만 명시
return { variables: { __modals: { '훈련세부': 'dismissible' } }, ... };
```

이는 `/api/sessions/[id]/tools/[name]/route.ts`의 modalChanges 머지 로직 때문이다. 패치된 `__modals`의 항목만 `Object.entries`로 순회하면서 group-aware 로직을 적용하므로, 키가 빠져 있으면 그 모달은 *변경 의도가 없는 것*으로 판단해 기존 상태를 유지한다.

값 매핑:
- `true` → 필수 모달로 열기 (닫기 불가)
- `'dismissible'` → 닫을 수 있는 모달로 열기
- `false` / `null` → 닫기
