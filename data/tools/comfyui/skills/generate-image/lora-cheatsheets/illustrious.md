...
| `xlcon00_fairy_position.safetensors` | 0.5~0.7 | 페어리 포지션 (앉아서 무릎 위 암시 섹스) | `sitting on another, sitting on lap, hands on thighs, public nudity, implied-sex` | DL:536 👍87 |
| `Spitroast_Threesome.safetensors` | 0.5~0.7 | 스핏로스트 (양쪽 동시 삽입 쓰리섬) | `spitroast, threesome` | DL:116 👍18, 218MB |
| `Hanging_on_person-IL_NAI_PY.safetensors` | 0.5~0.7 | 사람 몸에 매달려 구속 + 키스 | `chained wrists, horizontal wooden stick, looking at another, kissing, chained` | DL:750 👍201, 218MB |
...
| `sysdeep_blowbang.safetensors` | 0.5~0.7 | 블로우뱅 (다중 자지 펠라) | `sysdeep_blowbang` | DL:405 👍84, 112MB |
...
| `Glory_Wall_with_Portrait.safetensors` | 0.5~0.7 | 글로리홀 / 벽 너머 삽입 + 초상화 컨셉 | `g10ryw4l1, glory wall, lower body only, pov doggystyle pose, through wall, portrait on wall, body writing, tally marks, creampie` | DL:507 👍68, 218MB |
| `mesubuta-v2_IL.safetensors` | 0.5~0.7 | 메스부타(암퇘지) 굴욕/낙인 컨셉 | `mesubutax female` | DL:172 👍35, 56MB |
| `Defeat_Against_Ropes.safetensors` | 0.5~0.7 | 복싱 링 로프에 패배 기댐 | `rope_defeat_style, defeat, exhausted, against the ropes, leaning on ropes, boxing ring, ring ropes, body supported by ropes, injured, bruised, sweat` | DL:78 👍30, 50MB |
...
| `SLBS_latex_bitchsuit.safetensors` | 0.5~0.7 | 무봉제 라텍스 펫슈트 (인견 구속) | `SLBS_concept, bitchsuit, mouth mask, covered mouth, black mask, mask gag, posture collar, black collar, covered nose, thighhighs over bodysuit, fused legs, fused arms, bodysuit, black bodysuit, shiny clothes` | DL:159 👍48, 50MB. 풀바디 라텍스 슈트 |
...
| `cum_container.safetensors` | 0.5~0.7 | 컴 컨테이너 (입/신발/용기에 정액 받기) | `cum container, blush, open mouth, tongue out, cum, cum in mouth, suggestive fluid, cum on tongue, holding shoe, hands up, naughty face, torogao` | DL:179 👍35, 218MB |
| `BukkakeSlider3.safetensors` | 0.5~0.7 | 부카케 슬라이더 (양 조절형) | 트리거 태그 없음 | ⚠️ 수동 다운로드 필요 (CivitAI 로그인 필수). DL:16 👍79. https://civitai.com/models/2567557 |
...
| `cleft_of_venus.safetensors` | 0.5~0.7 | 외음부 (Cleft of Venus) 클로즈업 | `cleft of venus, looking at viewer, blush, open mouth, full body, uncensored, lying, spread legs, on back, anus, legs up, foot focus, torogao` | DL:341 👍58, 218MB. LylahLabia와 다른 결 |
| `Stretchmarks_Concept.safetensors` | 0.4~0.6 | 살트임 (만삭/거유 보조 디테일) | `stretchmarks, belly, boobs, thighs` | DL:122 👍15, 218MB. 다른 LoRA와 조합용 |
...
| `Public_bath.safetensors` | 0.5~0.7 | 대욕장 / 공중 목욕탕 (김 + 다중 인물) | `Public bath` | DL:580 👍111, 198MB |

### 임신/번식

| LoRA 파일명 | 강도 | 용도 | 트리거 태그 | 비고 |
|---|---|---|---|---|
| `Breeding_thralls-IL_NAI_PY.safetensors` | 0.5~0.7 | 종마/사육/속박 컨셉 (다중 임신) | `2girls, large breasts, nipples, pussy juice, arms up` 포즈/구속 태그와 조합 | DL:1,757 👍369, 218MB. 임신 컨셉 주력 |
| `pregnantdressNmode_Illustrious.safetensors` | 0.5~0.7 | 헐렁한 드레스 + 만삭 | `dress, pregnant` | DL:477 👍87, 52MB |

### 노출/NTR

| LoRA 파일명 | 강도 | 용도 | 트리거 태그 | 비고 |
|---|---|---|---|---|
| `exhibitionist_NTR.safetensors` | 0.5~0.7 | 보여주기 노출 NTR | `exhibitionist_NTRV1` | DL:1,402 👍255, 218MB |
| `exhibitionism-illustrious.safetensors` | 0.5~0.7 | 일반 노출증 컨셉 | `exh1bitionism, covering mouth with own hand, pinching nipples from behind, people around` | DL:474 👍80, 112MB |
| `Charity_slut-IL_NAI_PY.safetensors` | 0.5~0.7 | 사진 찍히는 NTR (스마트폰 촬영 굴욕) | `taking picture, holding, covering eyes with own hand, covering eyes, smartphone` | DL:422 👍162, 218MB |

### 촉수/부패 시퀀스

| LoRA 파일명 | 강도 | 용도 | 트리거 태그 | 비고 |
|---|---|---|---|---|
| `tentacle_infestation_sequence.safetensors` | 0.5~0.7 | 촉수 산란/감염 (벨리 부풀림, 산란) | `tentacle_PJQB, parasite, tentacle in nipples, tentacle in anal, tentacle coming out from nipples, fertilization, impregnation, seedbed, giving birth, egg laying, pregnant` | DL:406 👍65, 56MB |
| `abyssal_ship_corruption_sequence.safetensors` | 0.5~0.7 | 함선 의인화 부패 (생체 캡슐, 기계 촉수) | `abyssal_MST, Vacuum tube, Bio-pod, Transparent Experimental Chamber, mechanical tentacles, tentacle handcuffs, tentacle sex, mind control, hypnosis, brain injection, projectile lactation, purple cum` | DL:199 👍24, 56MB |
