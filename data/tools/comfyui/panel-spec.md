...
> ⚠️ **필수 모드(`true`) 모달·풀스크린 패널은 반드시 패널 내부에 상태 해제 트리거를 포함해야 한다.** 시스템 X 버튼이 없고 ESC도 막혀 있어서, 패널 내부에 진행 버튼(`__panelBridge.sendMessage(...)`) / 명시적 닫기 버튼(`__panelBridge.closeModal('이름')`) / 상태 전환 액션이 **없으면 유저가 영구히 갇힌다**. 풀스크린은 배경 대화창마저 안 보이므로 특히 치명적이다.
>
> 체크리스트:
