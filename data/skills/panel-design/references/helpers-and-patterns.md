...
### Form 컨트롤 (select / dropdown) 다크 테마

**기본 원칙:** 페르소나는 `layout.json`의 `theme` 섹션만 설정하면 *모든* `<select>` (닫힌 컨트롤 + 펼친 드롭다운 팝업 + 호버/선택 하이라이트)가 자동으로 그 톤을 따른다. **패널 CSS에서 select 색상을 따로 박을 필요가 없다.**

**작동 원리:**
- `PanelSlot.tsx`의 `PANEL_BASE_STYLE`이 모든 select에 `appearance: base-select` (Chrome 135+ customizable select 사양) + `::picker(select)` 옵트인을 자동 주입.
- 색상은 모두 CSS variables로 참조: `var(--bg)`, `var(--text)`, `var(--accent)`, `var(--accent-glow)`, `var(--border)`, `var(--text-dim)`.
- 이 변수들은 `useLayout.ts`가 `layout.json`의 `theme.*`를 document root에 주입한 것. Shadow DOM은 host를 통해 자동 상속.

**layout.json `theme` 토큰과 select 매핑:**

| 토큰 | 변수 | 사용처 |
|---|---|---|
| `bg` | `--bg` | 닫힌 select 배경, 옵션 기본 배경, 선택 항목 텍스트 |
| `text` | `--text` | 텍스트 색 |
| `textDim` | `--text-dim` | optgroup, picker icon, disabled |
| `border` | `--border` | 모든 테두리 |
| `accent` | `--accent` | **현재 선택된 항목** 배경 |
| `accent` (자동 0.35알파) | `--accent-glow` | **호버/키보드 네비 하이라이트** 배경 |
...
이거 하나로 모든 select가 페르소나 톤. 패널 HTML에는 select 관련 색상 CSS를 *적지 않는다.* 크기·폰트 조정만 패널에 남김:

```css
.my-select { font-size: 11px; }  /* 색은 layout.theme이 처리 */
```

**개별 select에 톤을 달리하고 싶을 때만** override:

```css
.special-select {
  background: var(--surface, #2a1818);  /* layout 토큰 위에 살짝 변경 */
}
.special-select option:checked {
  background: #ff6080;  /* 특정 select만 강조 */
}
```

**브라우저 호환:**
- Chrome/Edge 135+ (2025-05~): 풀 컨트롤. 권장.
- Chrome/Edge <135: `@supports not (appearance: base-select)` fallback로 닫힌 select와 OS 다크 모드까지는 자동 처리. 호버/네비 하이라이트는 시스템 색.
- Firefox/Safari: 미지원, native fallback.

**핵심 메시지:** *layout.theme만 설정하라.* 패널 CSS에 select 색상 박지 마라.
- Safari: macOS는 OS 다크 모드면 자연스러움, 명시 처리 권장
