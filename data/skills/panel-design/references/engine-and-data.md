...
| `omit_when_empty` | `true`면 값이 `null` / `""` / `0` / `false` / 빈 배열일 때 STATE 헤더에서 항목 자체를 생략. 빈 값이 의미 없는 임시 변수(현재 활동/의도/경고 등)에 박는다. |
| `omit_when_zero` | `true`면 값이 `0`일 때만 생략 (문자열은 표시). 카운터·누적 수치(0 = 미해금/없음)에 박는다. |
| `omit_when_false` | `true`면 값이 `false`일 때만 생략. boolean 플래그(임신 여부, 요구사항 등)에 박는다. |

> `omit_when_empty`가 `true`면 `omit_when_zero` / `omit_when_false`는 무시된다 (이미 빈 범주에 0/false가 포함됨). 셋 중 가장 *느슨한* 하나만 박는 게 원칙.
...
### STATE 헤더 다이어트 — opt-in 생략 플래그

매 턴 AI에 주입되는 `[STATE]` 라인은 토큰 비용·집중도 양쪽에 영향을 준다. 항상 채워지지 않는 변수는 `omit_when_*` 플래그로 빈 상태일 때 항목 자체를 빼는 게 원칙이다.

| 케이스 | 권장 플래그 |
|--------|-------------|
| `current_activity`, `current_intensity` 같이 *진행 중*에만 값이 차는 임시 변수 | `omit_when_empty: true` |
| `dark_reputation`, `hidden_marks_count` 처럼 *0 = 비활성/미해금*인 카운터 | `omit_when_zero: true` |
| `pregnant`, `pregnancy_required` 같은 boolean 플래그 | `omit_when_false: true` |
| 패널에 항상 표시되어 AI가 STATE에서 다시 볼 필요 없는 메타 (의뢰 ID, 결제 액수 등) | hint-rules에서 **항목 자체를 삭제** |
| 같은 정보를 여러 변수로 중복 노출 (개별 스탯 + summary 한 줄) | 결정에 직접 영향 없는 쪽을 hint-rules에서 삭제, summary만 남김 |

> 헤더 길이가 한 화면을 넘기 시작하면 다이어트 시그널. 위 표 순서대로 검토.
