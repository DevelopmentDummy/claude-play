...
7. RP 세션에서 사용할 스킬을 설계한다
   - 서비스 스케줄러, 자동 실행 루프, 배치 러너, 큐 워커, start/stop/restart/inspect 제어, 장기 실행 자동화 등록이 필요하면 `scheduler-registration` 스킬을 사용하고, 브라우저 로컬 루프가 아니라 서비스 런타임 관점에서 설계한다
...
**레이어 책임 경계 원칙 (엔진/패널/AI가 있는 페르소나 필수):** `panel-design` 스킬의 "아키텍처 원리 — 레이어 책임 경계" 섹션을 반드시 따라라. 요지:
- **엔진은 상태의 단일 소유자.** 상태 변경 호출자는 패널 레이어에 한정한다. AI는 엔진을 직접 호출하지 않고 헤더를 읽어 서사만 쓴다.
- **단일 실행 경로.** 같은 연산을 여러 레이어가 부를 수 있는 구조를 만들지 마라. 시간 진행·자원 증감 등은 엔진 액션 내부로 캡슐화하고 상위 레이어는 그 액션만 부른다.
- **부작용의 투명성.** 상태 변경은 `[STATE]` 스냅샷 + `[ACTION_LOG]` + `queueEvent`로 주입한 도메인 헤더로 AI에게 노출한다. 노출되지 않은 변경은 AI가 모르며 → 재실행·상태 불일치의 원인이 된다.
- **패널 액션 커버리지.** 상태를 바꾸는 모든 UI 버튼·다이얼로그 경로는 `registerAction`으로 등록하고 `executeAction`으로 실행하라. 인라인 `runTool` 호출은 AI 시야 밖의 유령 동작이 된다.
- 세션 `CLAUDE.md` / `session-instructions.md`에 "AI는 내레이터, 패널이 엔진을 돌린다" 원칙을 1~2 문단으로 명시하라. AI가 `[STATE]`·`[ACTION_LOG]` 헤더를 사실의 원본으로 읽고, 유저 자연어 메시지를 재실행 명령으로 해석하지 않도록.
...
**Choice 객체 전체 스키마 (모든 필드):**
```json
{
  "text": "버튼 표시 텍스트",      // 필수
  "score": 0,                      // 선택, 우선도(0이 기본)
  "dry": true,                     // 선택, AI 턴 소비 없이 UI/상태만 변경
  "actions": [                     // 선택, 클릭 시 실행할 도구 호출 목록
    { "tool": "engine", "action": "...", "params": { ... } }
  ]
}
```

**🚨 `dry` 플래그는 choice 레벨에 둬야 함, action 안에 두면 무시됨.** 디스패처(`ChatInput.tsx`)는 `choice.dry`를 읽고 `action.dry`는 검사하지 않는다. AI가 잘못된 형태로 학습/복사하면 인라인 모달 토글 버튼 같은 UI 전용 액션이 매번 AI 턴을 소비해서 무한 응답 루프가 생긴다.

```
❌ { "text": "...", "actions": [{ "tool": "engine", "action": "open_modal", "dry": true, "params": { ... } }] }
✅ { "text": "...", "dry": true, "actions": [{ "tool": "engine", "action": "open_modal", "params": { ... } }] }
```

**`dry: true`를 쓰는 경우:**
- 모달 열기/닫기 같은 UI 토글 전용 버튼
- 패널 활성화·뷰 전환 등 상태 표시만 바꾸는 액션
- 서사 응답 끝에 다음 패널로 이동하는 인라인 진입 버튼 (게임형 페르소나에서 흔함)

**`dry: false` (기본, 생략 가능)인 경우:**
- 게임 진행/상태 변화가 동반되는 액션 (가챠, 구매, 호감도 변화 등) — AI가 그 결과를 서사로 받아써야 자연스러움
...
## Lifecycle Hooks — 페르소나 → 세션 전파 규칙

페르소나 템플릿(`data/personas/{name}/`)에 새 hook이나 설정 파일을 추가하면, **이미 생성된 세션은 다음 Open 시점에 자동으로 받음** (additive mirror — 페르소나에만 있고 세션에 없는 파일만 1회 복사). 진행 중인 세션이 라이브 상태일 땐 반영되지 않으니, 세션을 닫고 다시 열어야 활성화된다.

**기존 세션 파일은 절대 덮어쓰지 않음** — RP 상태(`variables.json`, `chat-history.json` 등)는 보호됨. 사용자가 새 hook을 활성화하려면:
1. 빌더에서 페르소나 템플릿에 hook 파일 + 설정 파일 추가
2. 사용자에게 안내: *"기존 세션을 닫고 다시 열면 자동 활성화됩니다"*

새 세션 생성 시엔 페르소나의 모든 파일이 한 번에 복사되므로 추가 작업 불필요.
...
**생성 절차:**
1. 모든 파일 생성이 완료된 후, `character-tags.json`을 생성한다 (persona.md의 외형을 Danbooru 태그로 변환). **구조와 태그 작성 규칙은 `generate-image` 스킬의 "캐릭터 일관성 시스템" 섹션을 반드시 따르라** — 특히 `identity` / `accessories` / `outfit_*` 3단계 분리가 필수다.

   **⚠ 색상 지정 필수 — 모든 항목에 색상을 반드시 명시하라.**
   색상이 빠지면 이미지 모델이 매번 다른 색을 생성하여 일관성이 완전히 깨진다.
   - 의상: `skirt` ❌ → `navy pleated skirt` ✅, `blouse` ❌ → `white off-shoulder blouse` ✅
   - 장신구: `ribbon` ❌ → `red silk ribbon` ✅, `earrings` ❌ → `gold hoop earrings` ✅
   - 금속/보석: `buckles` ❌ → `silver buckles` ✅, `pendant` ❌ → `blue gem pendant` ✅
   - 머리/눈: `hair` ❌ → `platinum blonde hair` ✅, `eyes` ❌ → `deep blue eyes` ✅
   - 소재+질감도 포함: `gloves` ❌ → `black leather elbow gloves` ✅
...
- [ ] **기존 라이브 세션이 있는 페르소나에 새 hook/설정 파일을 추가했다면** — 사용자에게 "세션을 닫고 다시 열면 자동 적용됨"을 안내했는가? (Open 시 페르소나→세션 additive mirror로 새 파일이 자동 복사됨, 기존 파일은 덮어쓰지 않음)
