...
9. 세션 시작 시 보여줄 오프닝 메시지(첫 인사/등장 묘사)를 정한다
10. 모든 것이 정리되면 파일을 생성하고, 사용자에게 확인을 받는다
11. 이미지 생성용 체크포인트를 선택한다 (아래 체크포인트 선택 섹션 참조)
12. 캐릭터 프로필 이미지를 생성한다 (아래 프로필 이미지 섹션 참조)
13. 캐릭터 음성을 설정한다 (아래 음성 설정 섹션 참조)
