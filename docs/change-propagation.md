...
| 외부 MCP(/mcp/external) 노출 툴 추가/변경 | `src/lib/external-mcp/registry.ts` (`EXTERNAL_TOOLS`) + `docs/external-mcp.md` (툴 표) + 스킬팩 영향 시 `scripts/setup-external.mjs` (`CURATED_SKILLS`) 및 `scripts/external-package/skills/`. 반영은 서버 재시작(stateless — 세션 re-open 불필요). 노출 금지: fire_ai/run_tool/policy_*/bridge_* 등 세션·오케스트레이션 툴 |
...
| provider 은퇴/비활성 또는 모델 선택기 변경 | `ai-provider.ts` (`providerFromModel` 라우팅 + `MODEL_GROUPS`) + `.env.example` + `docs/infrastructure.md`. 예: Gemini CLI 은퇴 — `NEXT_PUBLIC_DISABLE_GEMINI=true`면 gemini-* 모델을 Antigravity로 투명 라우팅. **모델 id 목록 자체는 `MODEL_GROUPS`가 단일 소스** — 선택기 3곳(StatusBar 빌더/세션, PersonaStartModal), 프로바이더 기본값(`DEFAULT_MODELS`/`resolveBuilderModel`), 빌더 메타 프롬프트(`listBaseModelIds()` → `getBuilderPrompt`의 `{{validModels}}`)가 전부 따라오므로 `builder-prompt.md`를 손으로 고치지 말 것. 예외: MCP `.mjs` 도구 description의 모델 예시는 별도 프로세스라 수동(검증에 미사용) |
