...
`OPENAI_IMAGE_BACKEND=codex`(기본)는 `codex exec`의 빌트인 image_gen으로 렌더 (건당 $0, 느림 — 이미지당 codex 에이전트 풀 턴, 플랜 rate limit 소모). 함정: ① `OPENAI_IMAGE_MODEL`은 **오케스트레이션 모델**(gpt-5.5 등)이지 이미지 모델이 아니다 (api 백엔드 전용, codex 백엔드 미사용) — `gpt-image-*`를 넣으면 렌더러로 간주하고 오케스트레이션을 gpt-5.5로 폴백하는 자가 교정이 있다 (렌더러 핀은 `OPENAI_IMAGE_TOOL_MODEL`) ② 빈 temp cwd에서 실행 (AGENTS.md 오염 방지), prompt는 stdin (긴 argv는 shell:true에서 깨짐), child env에서 `OPENAI_API_KEY` **삭제** (구독 전용 렌더 강제) ③ `$CODEX_HOME/generated_images/`에서 mtime 스냅샷 diff로 출력 png 하베스트 — 동시 생성 시 오귀속 가능 (싱글유저라 수용). **🚨 파일명 규칙이 codex 버전에 따라 바뀐다** — 0.144.6 실측 `call_<toolCallId>.png`, 구버전 `ig_<id>.png`. 2026-07-28 이 변경으로 "생성은 성공했는데 하베스트 0장 → `Codex produced no image`" 무증상 실패가 발생했다 (이미지는 `generated_images/<uuid>/`에 멀쩡히 존재). 같은 증상이 재발하면 **먼저 `ls ~/.codex/generated_images/*/`로 실제 파일명을 확인**하고 `IG_OUTPUT_RE`를 넓혀라. codex 업그레이드 후 이미지가 안 나오면 1순위 용의자.

### 5.6 스킬 전파는 세션 open 시점
페르소나 `skills/`도 Open의 `refreshToolSkills(sessionDir, persona)`에서 갱신된다. 세션 측 변경 파일은 `.skill-backups/<provider>/skills/`에 내용 해시별 백업 후 원본으로 교체하며, 세션 전용 파일/원본에서 삭제된 파일은 지우지 않는다. 글로벌 도구 스킬의 동명 우선순위는 유지한다. 심볼릭 링크 경로는 따라가지 않는다. 상태 파일·일반 additive mirror 정책과 혼동하지 말 것. 실행 중 프로세스를 재사용한 경우 파일 갱신만으로 CLI 스킬 캐시까지 새로 고쳐졌다고 단정하지 않는다.
...
- **실측 (2026-08-18)**: 합성 테스트 — 310초 지연 응답에 전역 fetch는 305.6초에 실패, `longRequest`는 310.0초에 성공. 실경로 — 외부 MCP로 H3 Turbo 1152×640 15초물을 async 호출해 **1271초(21분) 완주**(h264+aac mp4). 절벽 통과가 GPU Manager 내부까지 포함해 확인됐다.
