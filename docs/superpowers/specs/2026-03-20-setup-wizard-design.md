...
7. **포트 설정** — 메인 포트 입력 (기본값: 3340). TTS = 메인+1, GPU Manager = 메인+2로 자동 할당.
8. **.env.local 생성** — 없을 때만 기본 템플릿 생성 (PORT 포함). 이미 존재하면 스킵.
9. **포트 충돌 검사** — 메인, 메인+1, 메인+2 세 포트. 사용 중이면 프로세스 정보와 함께 경고.
10. **data/ 디렉토리 초기화** — `personas/`, `sessions/`, `profiles/`, `tools/` 생성.
11. **완료 메시지** — `npm run dev`(개발) 또는 `npm run build && npm run start`(프로덕션)로 시작 안내.
