...
- **Session memo**: 같은 페르소나 세션을 로비에서 구분하기 위한 한 줄 메모. `session.json`에 수동 메모 `memo`와 AI 자동 요약 `autoMemo`(+ 갱신 시각 `autoMemoAt`)를 **분리 저장**해 자동 갱신이 사용자 메모를 덮어쓰지 않는다. 로비 카드 표시 우선순위는 `memo → autoMemo → 최근 유저 발화 프리뷰`(`listSessions()`가 `chat-history.json`에서 계산해 `memoFallback`으로 실어 보냄). 자동 갱신은 `variables.json`의 `__session_memo_counter`를 비-OOC assistant 턴마다 증분하다 10턴째에 `[MEMO]` 이벤트 헤더를 큐잉하고, 세션 AI가 `bridge_set_session_memo` MCP 도구로 `autoMemo`를 저장한다. 기본 ON이며 `session.json`에 `"memoAuto": false`로 끈다.
