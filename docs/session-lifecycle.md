...
Soft-delete cancels active `fire_ai` jobs owned by the target directory (including descendants) after closing the live instance and before orphan agy reap / rename. Cancellation clears the timeout and suppresses completion hooks, notifications, auto-resume, and pending readiness dispatch; sibling sessions remain active. Background jobs from a previous server process are not tracked by this in-memory cleanup. Regression: `npx tsx --test src/lib/background-session.test.ts`.

Optional persona MCP servers are merged by runtime config writers on open when `runtime-mcp.json` matches the machine-local `data/mcp-trust.json` approval. New sessions receive declarations through file copying; existing copies follow explicit sync, then reopen. See [persona MCP](specs/persona-mcp.md) for lifecycle and Blender worker boundaries.
...
2. **Open**: `POST /api/sessions/[id]/open` — provider별 AI 프로세스 spawn, PanelEngine 시작. spawn 전에 `refreshToolSkills(sessionDir, persona)`로 페르소나 스킬을 4종 CLI skills에 갱신하고 글로벌 스킬을 마지막 적용한다. 달라진 기존 페르소나 스킬 파일은 `.skill-backups/<provider>/skills/`에 내용 SHA256별 바이트 백업 후 교체한다. 세션 전용 파일/원본에서 삭제된 파일은 자동 삭제하지 않는다. panel-spec.md 갱신 및 panel-action 메타 markdown을 시스템 프롬프트에 주입한다. 일반 페르소나 신규 파일(hooks, opt-in 설정 등)은 additive mirror만 적용하며 기존 RP 상태는 보존한다. 실행 중 AI 프로세스를 재사용하는 페이지 새로고침에서는 스킬 파일 갱신과 CLI의 스킬 목록 재탐색을 구별한다. 메인 프로세스 spawn 완료 후 `subAgents.spawnAll()`로 상시 서브에이전트 기동 (`subagents.json` 존재 시)
...
- Builder commentary is checkpointed to `.claude/history-draft.json` every 250 ms while text arrives, and immediately on a completed content block/assistant message, process exit, instance destruction, or restart API call. A forced kill can still lose the last uncheckpointed fraction of a second; this is not a durable job runner.
- The live draft stays out of `chatHistory` to avoid duplicate streaming bubbles on history reload. On reopening an idle instance it is merged once by stable message ID, saved to `chat-history.json`, then removed. Normal completion/cancellation uses the same ID. Mid-turn user interjections split the assistant turn (see 턴 중 개입 below) — the frozen prefix is committed as its own history entry, so the draft never spans an interjection.
- History writes use atomic replacement and preserve UTF-8 BOM. Sequence IDs use the largest saved numeric suffix, not array length. Invalid history is retained and further writes are blocked rather than silently replacing it with an empty array.
- Draft capture is builder-only; RP result filtering remains unchanged. Tests: `npx tsx --test src/lib/history-storage.test.ts`. Deployment smoke: stream builder commentary, invoke restart API, reopen, verify one recovered assistant message and subsequent unique IDs. Do not rebuild a live production `.next/`.
