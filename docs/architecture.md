...
| `runtime-config.ts` | Per-runtime config file emitter (`.claude/settings.json`, `.mcp.json`, `.codex/config.toml`, `.gemini/settings.json`, `.agents/mcp_config.json` for Antigravity, policy-context.json). `ensureClaudeRuntimeConfig()` rewrites all of them on every session open, merging locally approved persona stdio MCP declarations. |
| `persona-mcp.ts` | Validates `runtime-mcp.json` and machine-local SHA-256 approvals in `data/mcp-trust.json`; expands project/app paths and rejects reserved names/env keys. Missing/unapproved manifests fail closed. See [persona MCP](specs/persona-mcp.md). |
...
| `soft-delete.ts` | Shared soft-delete flow for sessions and builder personas: close live instance → cancel directory-owned `fire_ai` jobs without completion hooks → `killAgyForDir()` orphan reap → rename into `data/deleted_*` with Windows-lock retry. |
| `fs-mirror.ts` | Recursive copy / additive mirror and content-addressed backup refresh for persona skills on Open; skips transient `background-*.log`. `fs-mirror.test.ts` covers byte preservation, idempotence, symlinks and SessionManager refresh integration. |
...
| `pipeline-scheduler.ts` | Per-session polling loop that periodically runs the persona's custom `pipeline` tool with `{ action: "scheduler_tick" }` (via `runSessionTool`), then dispatches resulting notifications. `start/stop/getPipelineSchedulerState()`. |
| `history-storage.ts` | Atomic UTF-8/BOM-preserving history writes, validation, stable numeric IDs, and idempotent builder assistant-draft recovery. |
...
| `antigravity-process.ts` | Antigravity (`agy.exe`) provider — the de-facto Gemini replacement. Persistent piped headless process (`agy --input-format stream-json --output-format stream-json -p ""`), same shape as `claude -p`. Same EventEmitter shape as the others. Details below. |
| `antigravity-pid-registry.ts` | Persists agy.exe PIDs to `data/.runtime/agy-procs.json` (`recordAgyPid()`); `killAgyForDir()` reaps orphans on session/persona delete — an agy child that outlives a crashed/restarted server otherwise blocks the directory rename with EBUSY. |

#### Antigravity runtime notes

- **Transport — headless stream-json over pipes (agy ≥1.2).** stdin: one NDJSON line per turn, `{"event":"user","message":{"content":[{"type":"text","text":…}]}}` (a plain-string `message` or `content` is rejected). stdout: `init` (`conversation_id` → `sessionId`), `step_update` (`step_type:"agent_response"` carries `text_delta`; also `tool` / `subagent` / `system_message`), `result` (`status` `SUCCESS`/`ERROR`, `error`). `-p ""` is mandatory — `-p` needs an argument and stream-json input rejects a command-line prompt. `--print-timeout 720h` because it bounds the headless wait. Resume = `--conversation <id>`; model = `--model <slug>` from `agy models`.
- **Why not the old LS polling.** Until 2026-09 the bridge spawned agy via PowerShell `Start-Process` (bubbletea TUI needs a console) and polled its in-process Language Server over ConnectRPC. agy 1.2.2 added a CSRF interceptor (`x-codeium-csrf-token`, token generated in-process and not exposed) so every RPC returned `401 missing CSRF token` → init failed → `send()` reported "not initialized". Pipe mode needs neither a TTY nor the token.
- **Turn bookkeeping.** agy queues stdin lines that arrive mid-turn and runs them as the next turn, so each `result` pops one entry of `turnQueue`. The primer (new conversations only) is the first queued turn and its output is suppressed; `steer()` just queues another user turn and the consumer sees a single `result` once the queue drains. `step_update`s arriving with an empty queue are an async-tool wake-up turn, emitted live and closed with `result.spontaneous: true`. Steps from other conversations (sub-agents) are ignored.
- **Model slugs are version-dependent — never hardcode.** `resolveModelSlug()` parses `agy models` (`slug\tDisplay Name`, 10-min cache) and picks the newest generation of the requested tier (`Flash (High)`, `Pro (Low)`, …).
- **MCP.** Headless agy ignores workspace `.agents/` (config, plugins, skills) and reads only the global `~/.gemini/config/mcp_config.json`. At spawn the bridge merges an env-less `claude-play` entry into that global file and passes the session's `claude-play` env (read from `.agents/mcp_config.json`) to the agy process; the MCP child inherits it and runs with the session dir as cwd. Persona-declared MCP servers are not reachable from agy. Servers start lazily on first `call_mcp_tool`.
- **Debugging**: per-session log `antigravity-stream.log` records every `[send]`/`[recv]` line and agy `[stderr]`; agy's own log is `~/.gemini/antigravity-cli/log/cli-*.log`.
