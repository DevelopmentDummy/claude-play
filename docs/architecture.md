...
- **MCP.** agy 1.2 documents only global and plugin-scoped `mcp_config.json`; the bridge registers per-session servers as a workspace plugin `.agents/plugins/claude-play/` (plus the legacy `.agents/mcp_config.json`). MCP servers start lazily on first tool use.
