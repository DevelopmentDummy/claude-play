...
export class ComfyUIClient {
  ...
  async buildPrompt(
    ...
    const prompt = await resolveWorkflow(pkg, params, {
      sessionDir,
      config: sessionDir ? readDirConfig(sessionDir) as Record<string, unknown> : undefined,
      models,
    });
    ...
    if (features.checkpoint_auto) {
      ...
      for (const node of Object.values(prompt)) {
        ...
        if (n.class_type === "CheckpointLoaderSimple") {
          ...
          if (resolvedCkpt === null) resolvedCkpt = resolveCheckpoint(models.checkpoints, this.checkpointName, sessionDir);
    ...
    validateCheckpointCompatibility(prompt, pkg, this.workflowsDir);
