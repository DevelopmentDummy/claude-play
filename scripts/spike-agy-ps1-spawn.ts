/**
 * Probe: launch agy via PowerShell .ps1 script file (avoiding -Command quoting hell).
 */
...
async function main() {
  const cwd = path.join(os.tmpdir(), "agy-ps1-spawn-probe");
...
Korean: 한국어 테스트 — multi-byte UTF-8`;
  ...
  const tempPrimerPath = path.join(tempDir, `primer-ps1-${Date.now()}.txt`);
  fs.writeFileSync(tempPrimerPath, primer, "utf-8");

  // Write a .ps1 script that reads primer + builds raw command line + spawns
  const escapePS = (s: string) => s.replace(/'/g, "''");
  // Start-Process with -ArgumentList <SINGLE STRING> passes the raw command
  // line. CommandLineToArgvW (Go's os.Args parser on Windows) follows MSDN
  // rules: \" escapes a literal quote, \\ escapes a backslash before a quote.
  // Steps: (1) double backslashes that precede quotes, (2) prefix \ to quotes,
  // (3) wrap result in "..." for whitespace tolerance.
  const psScript = `
...
`.trim();
  ...
  const tempScriptPath = path.join(tempDir, `spawn-${Date.now()}.ps1`);
  fs.writeFileSync(tempScriptPath, psScript, "utf-8");
  console.log(`[probe] wrote primer to: ${tempPrimerPath}`);
  console.log(`[probe] wrote ps1 to: ${tempScriptPath}`);
  ...
  try { fs.unlinkSync(tempPrimerPath); fs.unlinkSync(tempScriptPath); } catch { /* */ }
  ...
  console.log(`[probe] step[0] FULL JSON: ${JSON.stringify(traj.trajectory?.steps?.[0]).slice(0, 1500)}`);
  console.log(`[probe] cascade summary: ${JSON.stringify(all.trajectorySummaries?.[cid])}`);
  console.log(`[probe] EXACT match: ${step0Text === primer}`);
  console.log(`[probe] contains SENTRY-7: ${step0Text.includes("SENTRY-7")}`);
  console.log(`[probe] contains newline: ${step0Text.includes("\n")}`);
  console.log(`[probe] contains "codename?": ${step0Text.includes('"codename?"')}`);
